...
6. **マニュアル用スクリーンショットを生成（任意）**
    - 本マニュアルで使用しているスクリーンショットを最新版にしたい場合は、以下を実行してください。

       ```powershell
       python generate_manual_assets.py
       ```

    - 生成された PNG は `docs/images/` に保存されます。公開用の `images/` フォルダを更新したい場合は、必要なファイルをコピーして差し替えてください。
    - DICOM の元データは `img/` フォルダに配置します。このフォルダは `.gitignore` に登録済みのため、リポジトリやマニュアルには含まれません。
...
3. **オーバーレイ線**: Axial 上に、右側の断面位置を示す線が表示されます。
   - Sagittal 選択時: 縦線
   - Coronal 選択時: 横線
4. **コントロールパネル**: 右側の縦長領域に「断面ナビゲータ」「Windowing」「シリーズ情報」が並び、パラメータをまとめて変更できます。パネル幅は画像領域の約 1.8 倍に設定されており、スライダやプリセットが広く見やすく表示されます。
...
![断面の切り替え例](images/plane_switch.png)
