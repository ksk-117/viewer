...
- **Python**: 3.10 以上を推奨
- **必要なライブラリ**: `pydicom`, `numpy`, `pillow`, `PySimpleGUI`, `matplotlib`（`requirements.txt` で自動インストール）
...
2. **プロジェクトを取得**
   ...
   - 以下では `c:\Users\<ユーザー名>\develop\viewer` に配置したものとして説明します。

3. **PowerShell を起動し、プロジェクトに移動**

   ```powershell
   cd c:\Users\<ユーザー名>\develop\viewer
   ```
...
6. **マニュアル用スクリーンショットを生成（任意）**
   - 本マニュアルに掲載している画像を再生成したい場合は、以下を実行してください。

     ```powershell
     python generate_manual_assets.py
     ```

   - `docs/images/` に PNG ファイルが出力されます。
...
3. **オーバーレイ線**: Axial 上に、右側の断面位置を示す線が表示されます。
   - Sagittal 選択時: 縦線
   - Coronal 選択時: 横線
4. **操作タブ**: 「ナビゲーション」「Windowing」「情報」のタブを切り替えて各種パラメータを操作します。
5. **終了ボタン**: アプリケーションを終了します。
...
![ナビゲーションタブでの操作](images/plane_switch.png)

- **「ナビゲーション」タブ**の **Plane** コンボボックスで `Sagittal`（矢状断）または `Coronal`（冠状断）を選びます。
- 選択に応じて右側の画像が切り替わり、Axial 上のガイド線も縦線／横線に変わります。
- 面を切り替えると再構成スライダの範囲が自動で更新されます。

### 5.2 Axial スライス

- **「ナビゲーション」タブ**の `Axial スライス` スライダで元画像のスライス番号を変更します。
- スライダ下部に現在位置と全スライス数が表示されます（例: `120 / 300`）。
- **目的とする臓器や構造を中央に表示したい場合**: Axial スライダをゆっくり動かして該当スライスを探します。

### 5.3 再構成位置

- **「ナビゲーション」タブ**の `再構成位置` スライダは、選択中の断面（Sagittal または Coronal）の位置を指定します。
- 右側の再構成画像と、左側 Axial 上のガイド線が同時に更新されるため、断面位置の把握が容易です。
- **左右方向の位置関係を確認したい場合**は Sagittal、**前後方向を確認したい場合**は Coronal を選んでスライダを調整します。

### 5.4 Window Width (WW) / Window Level (WL)

![WW/WL 調整例](images/windowing.png)

- **「Windowing」タブ**で、画像のコントラストと明るさを調整します。
...
- **例**: 軟部組織を見やすくしたい場合は、WW を中程度に設定し、WL を臓器が最も見やすい位置に微調整します。
...
| 目的 | 推奨操作 |
| --- | --- |
| 骨構造を鮮明にしたい | WW を小さめ（例: 300 前後）、WL を高めに設定 |
| 軟部組織のコントラストを上げたい | WW を中くらい（例: 400〜600）、WL を徐々に調整 |
